...
public static class EventDelegateEditor
{
	...
	static public List<PropertyReferenceDrawer.Entry> GetMethods (GameObject target)
	{
		MonoBehaviour[] comps = target.GetComponents<MonoBehaviour>();

		List<PropertyReferenceDrawer.Entry> list = new List<PropertyReferenceDrawer.Entry>();
		...
		for (int i = 0, imax = comps.Length; i < imax; ++i)
		{
			...
			for (int b = 0; b < methods.Length; ++b)
			{
				...
				if (mi.ReturnType == typeof(void))
				{
                    ...
                    PropertyReferenceDrawer.Entry ent = new PropertyReferenceDrawer.Entry();
					ent.target = mb;
					ent.name = mi.Name;
					list.Add(ent);
				}
	...
	static public bool Field (Object undoObject, EventDelegate del, bool removeButton, bool minimalistic)
	{
		...
		if (del.target != null && del.target.gameObject != null)
		{
			...
			List<PropertyReferenceDrawer.Entry> list = GetMethods(go);
			...
			{
                PropertyReferenceDrawer.Entry entry = list[choice - 1];
				NGUIEditorTools.RegisterUndo("Delegate Selection", undoObject);
	...
	static public string[] GetNames (List<PropertyReferenceDrawer.Entry> list, string choice, out int index)
	{
		index = 0;
		...
		{
            PropertyReferenceDrawer.Entry ent = list[i];
