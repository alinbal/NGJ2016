...
public class UnitySynthTest : MonoBehaviour
{
    ...
    public float noteSpawnInterval = 1;
    private float _currentNoteSpawnInterval;
    private bool _canSpawnNote = false;

    public List<NoteCreator> noteCreators = new List<NoteCreator>();
    ...
    void Update()
    {
        if (!_canSpawnNote)
        {
            _currentNoteSpawnInterval += Time.deltaTime;
            if (_currentNoteSpawnInterval > noteSpawnInterval)
            {
                _currentNoteSpawnInterval = 0;
                _canSpawnNote = true;
            }
        }
    ...
    public void MidiNoteOnHandler(int channel, int note, int velocity)
    {
        if (channel == 0)
        {
            if (_canSpawnNote)
            {
                // noteCreators[channel % 5].CreateNote();
                var index = note % 5;// + rnd.Next(0, 4);
                                     //index = Mathf.Clamp(0, 4,index);
                                     //Debug.Log("index:" + index + "note:" + note);
                //UnityMainThreadDispatcher.Instance().Enqueue(noteCreators[index].CreateNote());
                _canSpawnNote = false;
            }
            //Debug.Log("NoteOn: " + note.ToString() + " Velocity: " + velocity.ToString() + "chanel:" + channel);
        }
    }
